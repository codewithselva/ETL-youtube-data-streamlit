...
def get_videos(playlist_id):
    ...
    try:
        while True:
            ...
            logger.debug(f"video_response =========> {playlistitem_response}")
            ...
            if not playlistitem_next_page_token:
                break
            logger.info(f"video details fetched successfully!")
        return video_list
    except Exception as e:
            ...
            if not next_page_token:
                ...
                if e.resp.status == 403:
                    ...
                    if error_reason == 'commentsDisabled':
                        logger.info(f"Comments disabled for the video ID: {video_id}")
                    else:
                        raise Exception(e)
                else:
                    raise Exception(e)
...
def load_channel_data_to_SQL(channel_info):
    ...
    try:
        ...
        if err.errno == errorcode.ER_DUP_ENTRY:
            logger.error(f"Duplicate entry for channel ID: {channel_info['_id']}. Skipping...")
        else:
            raise
    except Exception as e:
        logger.error(f"{channel_info['_id']}: {e}")
        raise Exception(e)
...
def load_playlist_data_to_SQL(channel_info):
    ...
    try:
        ...
        logger.error(err)
        if err.errno == errorcode.ER_DUP_ENTRY:
            logger.error(f"Duplicate entry for playlist ID. Skipping...")
        else:
            raise Exception(err)
    except Exception as e:
        logger.error(f"Error inserting playlists: {e}")
        raise Exception(e)
...
def load_video_data_to_SQL(channel_info):
    ...
    for playlist in playlists:
        for video in playlist['videos']:
            ...
            logger.debug(f'Total Duration (seconds): {duration}')
            ...
            logger.debug(f"*****load_video_data_to_SQL : video_data ==> {video_data}")
        ...
        for video in playlist['videos']:
            if video['comments']:
                for comment in video['comments']:
                    ...
                    logger.debug(f"comment_id ==> {comment_id}")
...
def show_channels_table():
    ...
    all_channels = channelDetails.find({})
    logger.debug(f"channel_info ==>  {all_channels}")
    if not all_channels :
...
def main():
    ...
    btn_extract = col1.button("Extract")
    btn_transform = col2.button("Transform")
...
##filter by channel name/id
    ...
    logger.info(f"selected_channel_id : {channel_id}")
    ...
    logger.info(f"statement => {cursor.statement}")
